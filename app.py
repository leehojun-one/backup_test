...
어제 표현: {yesterday_str}
"""
    length_rule = "전체 약 2400~2900자, 자연스러운 라디오 멘트체. 문법 해부 코너는 PDF 노트처럼 항목별로 또렷하게(• `영어부분` → 문법명: 한 줄 원리), 충분히 다루되 군더더기 없이."
    system_prompt = f"""
...
4. 🔬 미쓰리의 문법 해부 (이 방송의 하이라이트) — '한 문장 = 모든 문법'을 보여주세요:
   (가) 오늘의 공식이 들어간 '핵심 문장 1개'를 정하세요. 가능하면 호준 씨 영업 상황(창호·바닥재·견적·납기·협력사)으로, 두세 가지 문법이 함께 살아있는 문장으로.
       예: "We will create a new layout with the partner company by Friday." 처럼.
   (나) 그 한 문장을 영어로 들려주고 한국어 해석을 붙인 뒤, 문장 속 부분들을 아래 카테고리로 하나씩 짚어 해부하세요(해당되는 것만, 3~5개):
       · 뼈대·동사 (be동사/일반동사, 몇 형식인지)  · 조동사·부정 (확신·정중함의 온도)  · 관계대명사 (명사 뒤 '딱풀')
       · 접속사 (이정표: if/because/but…)  · 전치사 (점·면·공간, with=동행, by=마감선)  · 시제 (점=과거 / 선=현재완료)  · 관사 (a=아무거나 하나 / the=너도 알고 나도 아는 그것)
   (다) 각 짚을 때 "이 부분 '~'은 ○○예요 — 왜냐하면…" 식으로 원리를 한 줄로 쉽게. 한국인이 자주 틀리는 포인트가 있으면 'X문장 → O문장'으로 교정.
   (라) 마지막에 그 핵심 문장 전체를 끊어 읽기 + [[PAUSE]]로 따라하기.
